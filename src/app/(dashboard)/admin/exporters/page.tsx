...
export default function AdminExportersPage() {
    ...
    const handleToggleOperations = async (exporter: Exporter) => {
        try {
            const res = await fetch(`/api/exporters/${exporter._id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ operationsEnabled: !exporter.operationsEnabled }),
            });
            if (!res.ok) throw new Error('Update failed');
            toast.success(
                exporter.operationsEnabled
                    ? `${exporter.companyTradingName} removed from supervisor list`
                    : `${exporter.companyTradingName} added to supervisor list`
            );
            fetchExporters();
        } catch {
    ...
    const handleToggleBulkCheckout = async (exporter: Exporter) => {
        try {
            const res = await fetch(`/api/exporters/${exporter._id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ bulkCheckoutEnabled: !exporter.bulkCheckoutEnabled }),
            });
            if (!res.ok) throw new Error('Update failed');
            toast.success(
                exporter.bulkCheckoutEnabled
                    ? `Multiple check-out disabled for ${exporter.companyTradingName}`
                    : `Multiple check-out enabled for ${exporter.companyTradingName}`
            );
            fetchExporters();
        } catch {
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {paginated.map(exp => (
                                    <tr key={exp._id} className="hover:bg-emerald-50/30 transition-colors">
                                        ...
                                        <td className="px-4 sm:px-6 py-4">
                                            <button
                                                onClick={() => handleToggleOperations(exp)}
                                                title={exp.operationsEnabled ? 'Remove from supervisor list' : 'Allow in supervisor list (bypass request)'}
                                                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold transition-colors ${
                                                    exp.operationsEnabled
                                            ...
                                            >
                                                {exp.operationsEnabled
                                                    ...
                                                    : <><ZapOff className="w-3 h-3" /> Disabled</>}
                                            </button>
                                        </td>
                                        <td className="px-4 sm:px-6 py-4">
                                            <button
                                                onClick={() => handleToggleBulkCheckout(exp)}
                                                title={exp.bulkCheckoutEnabled ? 'Disable multiple check-out in supervisor portal' : 'Allow supervisors to select and check out multiple workers at once for this exporter'}
                                                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold transition-colors ${
